...
window.addEventListener("load", function(){
	...
	document.getElementById("get").addEventListener("click", function(){
		var option = {
			enableHighAccuracy: true,	//詳細な位置情報を取得
			timeout: 60*1000,		//タイムアウトまでは６０秒
			maximumAge:0
		};
	...
	function getPos(position){
		...
		var acc = position.coords.accuracy;		//経度緯度の誤差
		var altAcc = position.coords.altitudeAccuracy;	//高度の誤差
		var hd = position.coords.heading;			//方角
		ele.innerHTML = "緯度：" + lat + "<br>経度：" + lng + "<br>高度：" + alt + "<br>緯度経度の誤差：" + acc + "<br>高度の誤差：" + altAcc + "<br>方角：" + hd;
